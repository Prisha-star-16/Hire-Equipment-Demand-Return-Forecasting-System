...
# --------------------------------------------------------
# Category wise forecast container
# --------------------------------------------------------
...
for cat in df['EQUIPMENT_CATEGORY'].unique():
    ...
    # simple category forecast
    # using last known weekly demand

    if len(cat_ts) > 0:

        last_value = int(
            cat_ts['total_qty'].iloc[-1]
        )
